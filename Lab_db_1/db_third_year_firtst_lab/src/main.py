...
def gather_column_names():
    ...
    for name in filename_list:
        temp_df = pd.read_csv(f"../zno/{name}", sep=';', encoding='utf-8', on_bad_lines='skip', nrows=10)
...
def insert_chunk_into_sql_table(chunk):
    ...
    if table_exists(con, "zno"):
        ...
        opened_csv_file = open("temorary_csv.csv", encoding='utf-8')
...
def transorm_chunk_columns(chunk, df_columns_list, zno_year):
    ...
    if zno_year == 2021:
        ...
        chunk[cols_to_convert_to_int] = chunk[cols_to_convert_to_int].apply(lambda x: x.str.replace(',', '.')).apply(
            pd.to_numeric)
...
def run_sql_command():
    ...
    if table_exists(con, "zno"):
        ...
        BEGIN
             RETURN QUERY
                 select regname, min(mathball100) from zno
                 where mathteststatus = 'Зараховано' and year = needed_year 
                 group by regname;
    ...
    for name in filename_list:
        ...
        if file_counter < logged_numbers[0]:
            ...
            try:
                ...
                for chunk in pd.read_csv(f"../zno/{name}", sep=';', encoding='utf-8', on_bad_lines='skip',
                    ...
                    insert_chunk_into_sql_table(transformed_chunk)
                    print("inserted one chunk")

                chunk_counter = 0
                logging.info(
                    f'finished handling file: {name}; ' f'took time: {round(time.time() - start_time)} seconds')
